...
# Set outputs
echo "output1=this output1 is set by test-action-dockerfile" >> $GITHUB_OUTPUT
echo "output2=this output2 is set by test-action-dockerfile" >> $GITHUB_OUTPUT

# Add to PATH
echo "/foo/bar/test-action-dockerfile" >> $GITHUB_PATH

# Export variable
echo "MY_ENV=this env modified by test-action-dockerfile and input1 value is '${INPUT_INPUT1}'" >> $GITHUB_ENV
